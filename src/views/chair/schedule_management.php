...
<div class="bg-gray-50 min-h-screen">
                ...
                <form id="generate-form" class="space-y-6">
                    ...
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        <div class="flex items-center">
                            ...
                            <span class="text-sm font-medium text-gray-800">Current Semester: <?php echo htmlspecialchars($currentSemester['semester_name'] ?? 'Not Set'); ?></span>
                        </div>
                    </div>
        ...
        <div id="content-schedule" class="tab-content <?php echo $activeTab !== 'schedule-list' ? 'hidden' : ''; ?>">
            ...
            <script>
                ...
                function openAddModal() {
                    ...
                    document.getElementById('day-select').setAttribute('disabled', 'disabled');
                    document.getElementById('start-time').setAttribute('disabled', 'disabled');
                    document.getElementById('end-time').setAttribute('disabled', 'disabled');
                ...
                function editSchedule(scheduleId) {
                    ...
                    // Check and clean up existing modals
                    ...
                    modal.className = 'fixed inset-0 bg-black bg-opacity-50 backdrop-blur-md items-center justify-center z-1001 modal-overlay';
                    const modalContent = document.createElement('div');
                    modalContent.className = 'bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4 modal-content';
                    modalContent.innerHTML = `
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-semibold text-gray-900" id="modal-title">Edit Schedule</h3>
            <button onclick="closeModal()" class="text-gray-400 hover:text-gray-600">
                <i class="fas fa-times text-xl"></i>
            </button>
        </div>
        <form id="schedule-form" class="space-y-4" onsubmit="handleScheduleSubmit(event)">
            <input type="hidden" id="schedule-id" name="schedule_id" value="${schedule.schedule_id}">
            <input type="hidden" id="modal-day" name="day_of_week">
            <input type="hidden" id="modal-start-time" name="start_time">
            <input type="hidden" id="modal-end-time" name="end_time">
            ...
            <div>
                ...
                <input type="text" id="course-code" name="course_code" list="course-codes" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500" required oninput="syncCourseName()">
                <datalist id="course-codes">
                    ${Array.from(new Set(scheduleData.map(s => s.course_code))).map(code => `<option value="${code}"></option>`).join('')}
                </datalist>
            </div>
            ...
            <div>
                ...
                <input type="text" id="course-name" name="course_name" list="course-names" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500" required oninput="syncCourseCode()">
                <datalist id="course-names">
                    ${Array.from(new Set(scheduleData.map(s => s.course_name))).map(name => `<option value="${name}"></option>`).join('')}
                </datalist>
            </div>
            ...
            <div>
                ...
                <input type="text" id="faculty-name" name="faculty_name" list="faculty-names" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500" required>
                <datalist id="faculty-names">
                    ${Array.from(new Set(scheduleData.map(s => s.faculty_name))).map(faculty => `<option value="${faculty}"></option>`).join('')}
                </datalist>
            </div>
            ...
            <div>
                ...
                <input type="text" id="room-name" name="room_name" list="room-names" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500">
                <datalist id="room-names">
                    ${Array.from(new Set(scheduleData.filter(s => s.room_name).map(s => s.room_name))).map(room => `<option value="${room}"></option>`).join('')}
                </datalist>
            </div>
            ...
            <div>
                ...
                <input type="text" id="section-name" name="section_name" list="section-names" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500" required>
                <datalist id="section-names">
                    ${Array.from(new Set(scheduleData.map(s => s.section_name))).map(section => `<option value="${section}"></option>`).join('')}
                </datalist>
            </div>
    ...
    `;
                    ...
                    // Populate and sync initial values
                    ...
                    document.getElementById('room-name').value = schedule.room_name || 'Online';
                    ...
                    // Debug logs
                    console.log('Modal visibility:', modal.style.visibility, 'Modal offsetHeight:', modal.offsetHeight);
                    console.log('ModalContent offsetHeight:', modalContent.offsetHeight);
                    setTimeout(() => {
                        console.log('Modal hidden state after remove:', modal.classList.contains('hidden'));
                        console.log('Modal opacity:', window.getComputedStyle(modal).opacity);
                        console.log('Modal display:', window.getComputedStyle(modal).display);
                        console.log('Modal z-index:', window.getComputedStyle(modal).zIndex);
                        console.log('Modal pointer-events:', modal.style.pointerEvents);
                        console.log('Modal offsetHeight after render:', modal.offsetHeight);
                        console.log('ModalContent offsetHeight after render:', modalContent.offsetHeight);
                    }, 100);
                }
                ...
                // Updated handleScheduleSubmit with validation
                function handleScheduleSubmit(e) {
                    e.preventDefault();
                    const form = e.target;
                    const courseCode = form.querySelector('#course-code').value;
                    const courseName = form.querySelector('#course-name').value;
                    const facultyName = form.querySelector('#faculty-name').value;
                    const roomName = form.querySelector('#room-name').value;
                    const sectionName = form.querySelector('#section-name').value;
                    const startTime = form.querySelector('#start-time').value;
                    const endTime = form.querySelector('#end-time').value;

                    if (!courseCode || !courseName || !facultyName || !sectionName || !startTime || !endTime) {
                        showNotification('All fields except Room are required', 'error');
                        return;
                    }
                    if (new Date(`2000-01-01 ${endTime}:00`) <= new Date(`2000-01-01 ${startTime}:00`)) {
                    ...
                    if (scheduleId) body.append('schedule_id', scheduleId);
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                const updatedSchedule = {
                                    schedule_id: scheduleId || data.schedule_id,
                                    course_code: courseCode,
                                    course_name: courseName,
                                    faculty_name: facultyName,
                                    room_name: roomName || 'Online',
                                ...
                                if (scheduleId) {
                                    ...
                                    if (index !== -1) scheduleData[index] = updatedSchedule;
                                } else {
                ...
                ${formatTime(time[0])} - ${formatTime(time[1])}
                            ...
                            ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].forEach(day => {
                                ...
                                if (schedule) {
                                    ...
                                    const colorClass = colors[Math.floor(Math.random() * 5)] + ' border-l-4';
                                    cell.innerHTML = `<div class="schedule-card ${colorClass} p-2 rounded-lg border-l-4 mb-1 draggable cursor-move"
                        ...
                        data-year-level="${escapeHtml(schedule.year_level)}" data-section-name="${escapeHtml(schedule.section_name)}" data-room-name="${escapeHtml(schedule.room_name ?? 'Online')}">
                        <div class="flex justify-between items-start mb-2">
